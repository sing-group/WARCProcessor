...
public class GCSaveAction extends AbstractAction implements Observer {
	...
	public void init(IAppLogic logic, WarcGeneratorGUI view,
	...
	public void actionPerformed(ActionEvent e) {
		...
		appConfig.setNumSites(Integer.parseInt((String) panel
				.getNumSitesTField().getValue()));
		if (panel.getSpamHamRatioRBtn().isSelected()) {
			appConfig.setRatioIsPercentage(true);
			appConfig.setRatioPercentageSpam(Integer.parseInt(panel
					.getSpamHamRationValueTField().getText()));
		} else {
			appConfig.setRatioIsPercentage(false);
			appConfig.setRatioQuantitySpam(Integer.parseInt(panel
					.getSpamQuantityTField().getText()));
		}
	...
	public boolean validate(AppConfig config) {
		...
		try {
			...
			errors.append(Messages.getString("GCSaveAction.error.text"));
		}
